...
CREATE TABLE `user_playlist`  (
  ...
  `is_active` int NOT NULL DEFAULT 1,
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `user_playlist_fk1`(`user_id` ASC) USING BTREE,
  INDEX `user_playlist_fk2`(`playlist_id` ASC) USING BTREE,
  CONSTRAINT `user_playlist_fk1` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `user_playlist_fk2` FOREIGN KEY (`playlist_id`) REFERENCES `playlist` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB CHARACTER SET = utf8mb3 COLLATE = utf8mb3_general_ci ROW_FORMAT = Dynamic;
